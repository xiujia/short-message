...
int process_received(char *buf)
{
	...
	int tty_fd;
		...
		if(log_file==NULL)
			...
			//printf("send----\n");
			process_alarm_message(tty_fd,buf);
...
int process_alarm_message(int fd,char *message){
	...
	//首先读取第一行获取到短信号码
	printf("message==%s\n",message);
	//get_line ( message , buf , strlen(message) ) ;
	char* pucStart = message;
	char* pucTemp =strstr(pucStart,"->");//->为电话号码和短信内容分隔符，可以改成回车换行"\r\n"
	if(NULL ==pucTemp)
	{
		log_warn(log_file," message datatype error\n");
		return 0;
	}
	memcpy(buf,pucStart,pucTemp-pucStart);
	...
	pucStart =pucTemp+2;
	memcpy(buf,pucStart,strlen(message)-(message-pucStart));
		...
		if(strstr(msg.data,MSG_SYSINFO) != NULL)
			{
				...
				{
					sprintf(msg_info.TP_DA,"系统状态：mem:%.2f\%,cpu:%.2f\%,hard disk:%.2f\%",status.mem,status.cpu,status.disk);
					GSM_Send_PDUMessage(fd,&msg_info);	
					insert_send_msg_to_mysql(&msg_mysql,&msg_info);					
					return ;
				}
